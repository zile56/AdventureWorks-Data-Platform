"""Customer Department Data Ingestion Script"""
import sys, os
...
def ingest_customer_data(execution_date=None):
    logger.info("CUSTOMER DEPARTMENT DATA INGESTION")
    ...
    # API data
    for endpoint in DEPARTMENT_SOURCES['customer']['api_endpoints']:
        try:
            ...
            if data:
                ...
                object_path = get_landing_path('customer', table_name, execution_date)
    ...
    # CSV data
    for csv_file in DEPARTMENT_SOURCES['customer']['csv_files']:
        try:
    ...
    logger.info(f"CUSTOMER COMPLETE - Success: {results['success']}")
    return results
...
def main():
    ...
    results = ingest_customer_data(parser.parse_args().date)
