"""Finance Department Data Ingestion Script"""
import sys, os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import pandas as pd
import logging
from datetime import datetime
from utils.config import MINIO_CONFIG, FILE_PATHS, DEPARTMENT_SOURCES
...
def ingest_finance_data(execution_date=None):
    logger.info("FINANCE DEPARTMENT DATA INGESTION")
    ...
    for csv_file in DEPARTMENT_SOURCES['finance']['csv_files']:
        try:
            ...
            if os.path.exists(csv_path):
                ...
                object_path = get_landing_path('finance', table_name, execution_date)
    ...
    logger.info(f"FINANCE COMPLETE - Success: {results['success']}")
    return results
...
def main():
    ...
    results = ingest_finance_data(parser.parse_args().date)
